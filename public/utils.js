...
class InfoTemplate extends TemplateRoot {
	...
	createHeroInfoTemplate(hero) {
		const heroInfoTemplate = `<div class="hero" id="hero">
          ...
          <div data-slot="Main weapon" class="slot">
            ${hero.weapon ? this.getImage(hero.weapon.image, 'weapon') : ''}
          </div>
          <div data-slot="Shield" class="slot">
            ${hero.shield ? this.getImage(hero.shield.image, 'shield') : ''}
          </div>
          <div data-slot="Secondary weapon" class="slot">
            ${
							hero.accessory
								? this.getImage(hero.accessory.image, 'accessory')
								: ''
						}
          </div>
	...
	getAllEnnemyDescription(ennemys) {
      ...
      <div class="slot" data-slot=" Level ${
				ennemy.getLevel ? ennemy.getLevel() : ''
			}" >
        <h2 class="name">${ennemy.name} ${ennemy.life > 0 ? '' : 'DEAD'}</h2>
      </div >
      <div class="slot">
          <img src="${ennemy.image}" alt="heracles" class="${
				ennemy.life > 0 ? '' : 'dead'
			}">
      </div>
      <div data-slot="Attack" class="slot statistic">
...
class ArenaTemplate extends TemplateRoot {
	 ...
	 */
	makeTile(tile) {
		return `<img class="tile" src="${tile.image}" alt="" />`;
	}
	...
	makeMonsterImage(arena, index) {
        ...
        title="Distance to ${arena.hero.name} ${
				arena.getDistance
					? arena.getDistance(arena.monsters[index], arena.hero)
					: ''
			}" 
        class="monster ${
					arena.isTouchable
						? arena.isTouchable(arena.hero, arena.monsters[index])
							? 'touchable'
							: 'untouchable'
						: ''
				}" 
	...
	createArena(arena) {
      ...
      ${this.checkCards(arena, i, j)}
		...
		const arenaTemplate = `${
			arena.digArena
				? `<button class="btn btn-primary btn-shovel" id="digButton"><img src="./images/shovel.svg"><span>Dig</span></button>`
				: ''
		}
      <div class="map" style="--tiles-number: ${arena.size}" >
